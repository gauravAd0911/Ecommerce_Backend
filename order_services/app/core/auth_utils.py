...
def resolve_guest_user_id(guest_token: str) -> str:
    normalized = str(guest_token or "").strip()
    if not normalized:
        raise ValueError("Guest token is required.")
    return f"{GUEST_TOKEN_PREFIX}{normalized[:GUEST_TOKEN_MAX_LENGTH]}"
